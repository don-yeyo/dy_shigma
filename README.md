...
- **Residuos Industriales No Especiales (RINE)**: Registro y pesaje detallado de desperdicios orgánicos, inorgánicos de la marca de la empresa e inorgánicos generales por planta generadora. Incluye un switch interactivo para clasificar inorgánicos como *Recuperables* con desglose por material (Cartón, Metal, Cajones, Conos de Film Stretch, Aceite vegetal y Otros) y auto-cálculo reactivo del lote, limitando el registro de Cajones Rotos exclusivamente a Kilos. Si el residuo es de tipo "Inorgánicos Generales", el campo Sector se establece en null y se oculta automáticamente, habilitándose en su lugar un desplegable para seleccionar si es "Húmedo" o "Seco" únicamente si la Planta generadora es "Elguea Roman". Posee integración con un **Monitor de Capacidad de Bateas**, el cual permite a cualquier nivel de usuario ajustar de forma dinamica y en tiempo real la capacidad de cualquier batea en Kilos mediante un modal premium con controles numéricos finos, así como vaciar y reiniciar bateas (con manifiesto de salida obligatorio en estado *pendiente*). Al realizar la confirmación de recepción de una batea por parte de un supervisor o sysadmin, se exige ingresar obligatoriamente un **Número de Certificado** de hasta 30 caracteres.
- **Control de Depósito de Recuperables RINE**: Permite gestionar y monitorear el acopio físico de los residuos inorgánicos generales recuperables (Cartón, Metal, Cajones, Conos de Film Stretch, Aceite vegetal y Otros) almacenados en el depósito en lugar de las bateas tradicionales. Los supervisores y administradores pueden realizar despachos individuales por tipo de material registrando proveedor (con autocompletado en base a historial de localStorage), fecha, hora, número de manifiesto (opcional) y peso en balanza (obligatorio), lo cual coloca el despacho en estado *pendiente*. Asimismo, permite confirmar la recepción con número de certificado opcional e **implementa la funcionalidad de ajuste manual de stock por tipo de material** (ingresando kilogramos positivos o negativos y motivo) creando un lote de ajuste virtual RINE de control para auditoría.
